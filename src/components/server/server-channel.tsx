...
export const ServerChannel = ({ channel, server, role }: ServerChannelProps) => {
  ...
  return (
    <button
      onClick={() => {}}
      className={cn(
        'group p-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700 dark:hover:bg-zinc-700/50 transition mb-1 cursor-pointer',
        params?.channelId === channel.id && 'bg-zinc-700/20 dark:bg-zinc-700'
      )}
    >
      <Icon className="flex-shrink-0 w-4 h-4 text-zinc-500 dark:text-zinc-400 dark:group-hover:text-white" />
      <p
        className={cn(
